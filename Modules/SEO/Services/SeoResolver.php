...
class SeoResolver
{
    public static function for($model): array
    {
        ...
        $seoManager = app(\Modules\SEO\Services\Seo\SeoManager::class);
    ...
    protected static function registerSchemas($seoManager, $model, $data): void
    {
        ...
        if ($data['type'] === 'Article' && isset($data['article'])) {
            $seoManager->addSchema(new \Modules\SEO\Services\Seo\Schemas\ArticleSchema(
                headline: $data['title'],
                description: $data['description'],
                url: $data['url'],
                image: $data['og_image'],
        ...
        // 2. WebPage
        $seoManager->addSchema(new \Modules\SEO\Services\Seo\Schemas\WebPageSchema(
            title: $data['title'],
            description: $data['description'],
            url: $data['url'],
        ...
        if ($data['type'] === 'Service' && isset($data['service'])) {
            $seoManager->addSchema(new \Modules\SEO\Services\Seo\Schemas\ServiceSchema(
                name: $data['title'],
                description: $data['description'],
                url: $data['url'],
                image: $data['og_image'],
        ...
        if ($model instanceof \Modules\News\Models\News) {
            $crumbs[] = ['name' => 'News', 'item' => config('app.url') . '/news'];
        } elseif ($model instanceof \Modules\Services\Models\Service) {
            $crumbs[] = ['name' => 'Services', 'item' => config('app.url') . '/services'];
        }
        $crumbs[] = ['name' => $model->title ?? $model->name, 'item' => $data['url']];

        $seoManager->addSchema(new \Modules\SEO\Services\Seo\Schemas\BreadcrumbSchema($crumbs));
    }


    public static function staticPage(string $title, string $description = ''): array
    {
        $settings = \Modules\Settings\Models\Setting::whereIn('key', ['seo_default_title', 'seo_default_description', 'seo_default_og_image'])
            ->pluck('value', 'key');
