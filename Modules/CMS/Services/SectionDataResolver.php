...
class SectionDataResolver
{
    ...
    */
    protected function resolvePath($path)
    {
        ...
        // If it's an array (which shouldn't happen if model normalization works, but for safety), extract string
        if (is_array($path)) {
            $path = $path['url'] ?? $path['image'] ?? collect($path)->first(fn($p) => is_string($p));
        }
    ...
    public function resolve(PageSection $section): array
    {
        ...
        $data = match ($key) {
            ...
            'partner_list' => $this->resolvePartnerList($config),
    ...
    {
        // Resolve images: Maintain object structure but resolve paths within
        $images = collect($config['images'] ?? [])->map(function ($img) {
            if (is_array($img)) {
                if (isset($img['url'])) $img['url'] = $this->resolvePath($img['url']);
                if (isset($img['image'])) $img['image'] = $this->resolvePath($img['image']);
                return $img;
            }
    ...
    protected function resolvePartnerList(array $config): array
    {
        if (!Schema::hasTable('brands')) {
        ...
        $dbBrands = DB::table('brands')
            ->select('id', 'name', 'slug', 'thumbnail', 'image', 'website_url', 'category', 'is_featured')
            ->get();

        $brands = $dbBrands->map(function ($b) {
