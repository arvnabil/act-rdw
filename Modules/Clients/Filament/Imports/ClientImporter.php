...
namespace Modules\Clients\Filament\Imports;
...
class ClientImporter extends Importer
{
    ...
    {
        \Illuminate\Support\Facades\Log::info('ClientImporter: afterSave started for Record Service: ' . $this->record->id);
        \Illuminate\Support\Facades\Log::info('ClientImporter: Record current logo in DB: ' . $this->record->logo);
        ...
        $seoData = [
            ...
            'og_image' => \App\Helpers\ImageHelper::resolveImageFromUrl($this->data['og_image'] ?? null, 'seo/og', $this->record->slug, $this->record->seo?->og_image ?: $this->record->logo),
        ...
        \Illuminate\Support\Facades\Log::info('ClientImporter: Logo value from CSV: ' . ($logo ?: 'NULL'));
        ...
        if (!blank($logo) && (filter_var($logo, FILTER_VALIDATE_URL) || str_starts_with($logo, 'http'))) {
            ...
            if ($localPath) {
                \Illuminate\Support\Facades\Log::info("ClientImporter: Detected local URL '{$logo}', skipping download and using existing file: {$localPath}");
                $this->record->update(['logo' => $localPath]);
            } else {
                \Illuminate\Support\Facades\Log::info("ClientImporter: Detected external URL '{$logo}', starting migration to local storage.");
                try {
                    ...
                    \Illuminate\Support\Facades\Log::info("ClientImporter: Download response status: " . $response->status());

                    if ($response->successful()) {
                        $contents = $response->body();
                        \Illuminate\Support\Facades\Log::info("ClientImporter: Download successful, size: " . strlen($contents) . " bytes");
                        ...
                        if ($savedPath) {
                            \Illuminate\Support\Facades\Log::info("ClientImporter: Image processed and saved to: {$savedPath}");
                            $this->record->update(['logo' => $savedPath]);
                            \Illuminate\Support\Facades\Log::info("ClientImporter: Record updated with new logo path.");
                        } else {
                            \Illuminate\Support\Facades\Log::error("ClientImporter: ImageHelper returned NULL after processing.");
                        }
                    } else {
                        \Illuminate\Support\Facades\Log::warning('ClientImporter: URL download failed. Status: ' . $response->status() . ' URL: ' . $logo);
                    }
                } catch (\Throwable $e) {
                    \Illuminate\Support\Facades\Log::error('ClientImporter: Failed to download/process logo: ' . $e->getMessage());
                }
            }
        } else {
            \Illuminate\Support\Facades\Log::info("ClientImporter: Logo is either blank or NOT a URL. Skipping download.");
        }
    }
