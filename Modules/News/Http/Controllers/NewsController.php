...
class NewsController extends Controller
{
    public function index($categorySlug = null, $tagSlug = null)
    {
        ...
        if ($categorySlug) {
             $activeCategory = NewsCategory::where('slug', $categorySlug)->firstOrFail();
        ...
        // Handle Tag Filter
        if ($tagSlug) {
             $activeTag = NewsTag::where('slug', $tagSlug)->firstOrFail();
        ...
        $tags = NewsTag::orderBy('name')->take(20)->get();
        ...
        $page = \Modules\CMS\Models\Page::where('slug', 'news')->first();
