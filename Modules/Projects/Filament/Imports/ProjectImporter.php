...
namespace Modules\Projects\Filament\Imports;
...
class ProjectImporter extends Importer
{
    ...
    {
        \Illuminate\Support\Facades\Log::info('Importing Project ID: ' . $this->record->id . ' | Slug: ' . $this->record->slug);
        ...
        if (isset($this->data['thumbnail'])) {
            ...
            if (empty($thumbnailPath) || strtoupper($thumbnailPath) === 'DELETE') {
                ...
                if (str_starts_with($thumbnailPath, 'http')) {
                    ...
                    if ($localPath) {
                        \Illuminate\Support\Facades\Log::info("ProjectImporter: Detected local URL '{$thumbnailPath}', skipping download and using existing file: {$localPath}");
                        $this->record->update(['thumbnail' => $localPath]);
                        $contents = null; // Skip further processing
                    } else {
                        \Illuminate\Support\Facades\Log::info("ProjectImporter: Detected external URL '{$thumbnailPath}', starting migration to local storage.");
                        try {
                            ...
                            \Illuminate\Support\Facades\Log::info("ProjectImporter: Download response status: " . $response->status());

                            if ($response->successful()) {
                                $contents = $response->body();
                                $sourceType = 'URL';
                                \Illuminate\Support\Facades\Log::info("ProjectImporter: Download successful, size: " . strlen($contents) . " bytes");
                            } else {
                                \Illuminate\Support\Facades\Log::warning("ProjectImporter: URL download failed for {$thumbnailPath}. Status: " . $response->status());
                            }
                        } catch (\Throwable $e) {
                             \Illuminate\Support\Facades\Log::warning("ProjectImporter: URL download failed for {$thumbnailPath}. Error: " . $e->getMessage());
                        }
                    }
                }
                ...
                if ($contents) {
                    try {
                        ...
                        \Illuminate\Support\Facades\Log::warning("Project Image Processing failed: " . $e->getMessage());
                    }
                }
            }
        }
