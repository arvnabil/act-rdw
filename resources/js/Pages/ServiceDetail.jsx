...
export default function ServiceDetail({ service }) {
    ...
    const filteredRooms = filter === '*'
        ? service.rooms
        : service.rooms.filter(room => room.category.includes(filter.replace('.', '')));
                ...
                <div className="container">
                    <div className="row justify-content-center text-center">
                        <div className="col-xl-12">
                            <span className="sub-title">Reimagine your workspaces.</span>
                    ...
                    <div className="row">
                        <div className="col-xl-12">
                            <div className="case-menu mb-5 text-center">
                                <button
                                    className={`case4-btn ${filter === '*' ? 'active' : ''}`}
                                    onClick={() => setFilter('*')}
                                >
                                    All Brand
                                </button>
                                {categories.map((cat, index) => (
                                    <button
                                        key={index}
                                        className={`case4-btn ${filter === cat.value ? 'active' : ''}`}
                        ...
                        {filteredRooms.map((room) => (
                            <div className="col-12" key={room.id}>
                                <div className="row gy-30 align-items-center">
                                    <div className={`col-lg-6 ${room.id === 'small-room' ? 'order-lg-1' : ''}`}> {/* Alternating layout logic simulation */}
                                        <div className="about-image ab-img16 global-img">
                                            <img src={room.image} alt={room.title} className="w-100 rounded-20" />
                                        </div>
                                    </div>
                                    <div className={`col-lg-6 ${room.id === 'small-room' ? 'order-lg-0' : ''}`}>
                                        <div className="about-item style-16 ps-xxl-4 ms-xxl-4">
                                            <div className="about-content">
                                                <h3 className="box-title text-anime-style-2">
                                                    <Link href={`/services/${service.id}/${room.id}`}>{room.title}</Link>
                                                </h3>
                                                    ...
                                                    <div className="about-feature pe-xl-4">
                                                        <ul>
                                                            <li className="wow fadeInUp" data-wow-delay=".1s">
                                                                {room.capacity} <i className="far fa-user text-theme ms-1"></i>
                                                            </li>
                                                ...
                                                <div className="about-btn mt-30 wow fadeInUp" data-wow-delay=".6s">
                                                    <Link href={`/services/${service.id}/${room.id}`} className="th-btn btn-2 th-icon">
                                                        Get Started Now
